...
Small things I noticed but didn't want to gate the release on. Next
patch unless something else jumps the queue:

- `bonsai-cc export --format svg|gif` still lists those in `--help`
  even though they raise `ExportError("not implemented")`. Drop them
  from the help string.
- Top-level `--port` and `--no-browser` on `bonsai-cc` are shadowed
  by each subcommand's own copies, so `bonsai-cc --port 8080 web`
  silently uses port 0. Either thread them through or remove.
- Truncate `<home>/_install_smoke.jsonl` per `install-hook` run.
  Currently it appends forever (file is outside `journals/` so it
  doesn't affect anything — cosmetic).
...
- `[bcc-debug] page loaded` console line on the page fires even
  without `?debug=1`. Quieten it.
...
[Unreleased]: https://github.com/davvikq/bonsai-cc/compare/v0.2.0...HEAD
