...
:mod:`bonsai_cc.growth`. The architectural seam (DESIGN.md §1.1)
...
# Persistence cadence (DESIGN.md §6.2 + the user's
...
class GrowthRunner:
    ...
    and dropped -- DESIGN.md §6.3 punts multi-renderer support to v2.
    ...
    def _handle(self, ingested: IngestedEvent) -> None:
        ...
        if self._session_id is None:
            ...
            # current binding (DESIGN.md §10: "one session at a time;
...
def _maybe_recover_one(store: GardenStore, journal_path: Path) -> int:
    ...
    for rec in records:
        ...
        except Exception:  # noqa: BLE001 - drop malformed events
            pass
        next_idx += 1
