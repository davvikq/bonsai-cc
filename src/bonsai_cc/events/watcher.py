...
The hook writes directly to ``<home>/journals/*.jsonl`` (POSIX
``O_APPEND`` atomic; ``msvcrt.locking`` on Windows). When the
daemon is running, :class:`JournalWatcher` watches that directory
via ``watchfiles`` (inotify on Linux, FSEvents on macOS,
ReadDirectoryChangesW on Windows), tail-reads new lines as they're
appended, validates each via :func:`parse_event`, and publishes
:class:`IngestedEvent` to the bus.

What's NOT here:

* Journaling: the hook already wrote the line. This module is a
  consumer.
* Garden persistence: :class:`GrowthRunner` handles it from the bus
  side; the watcher does not cross that seam.

Per-file byte offsets are tracked so reconnect / restart never
re-publishes the same lines. The initial scan on startup processes
every existing journal from byte 0, catching up with whatever the
hook wrote while the daemon was down.
"""
...
# Per DESIGN.md §6.2 -- defensive against malformed payloads. Same
