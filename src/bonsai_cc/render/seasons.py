"""Seasonal + time-of-day overlays painted on top of the projected grid.

Render-time only; never mutates ``TreeState`` (overlays change with
the wall clock, state is event-derived). Season picks by session
duration (>=1h); time-of-day picks by current hour. Ambient cell
positions are seeded from session + current minute, so two renders
in the same minute paint identical sparkles.
"""
