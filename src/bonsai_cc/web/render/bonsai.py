"""``render_generic_bonsai`` -- the default theme.

The neutral, pleasant fallback that ships when no language is
detected. It must never look bad, never stand out, and obey every
proportional rule in DESIGN.md. Other themes' renderers deviate
from this shape (bamboo splits into stalks, sakura goes slender
literati, pine gets tiered foliage, etc.).

Strategy:

* Single S-curve trunk with three Bezier control points giving
  asymmetry. Bottom 30% stays near-constant width; the upper 70%
  tapers quickly.
* One subtle bark-side shadow stripe on the left of the trunk for
  depth, plus two short striation marks on the right.
* Branches emerge from the trunk at each ``Branch.attach_point``;
  swing roughly horizontal then arc gently upward at the tip.
* Foliage is clusters of 9 ellipse "leaves" with two-tone shading
  and deterministic jitter -- never single leaves.
* Top of the trunk gets the largest leaf cluster (the apex
  signature) so the trunk's pointed tip is never visible.
* Roots / offshoots are kept simple but present.
"""
