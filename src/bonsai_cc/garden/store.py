...
Concurrency note: SQLite's WAL mode lets the daemon write while the
``garden`` browser TUI reads concurrently -- exactly the v2 use case.
For v1 (one process), the busy_timeout = 5000 covers the rare lock
contention between the consumer task and a manual ``bonsai-cc list``
...
def render_final_ascii(
    ...
    *,
    ...
    ``bonsai-cc show`` prints. Stable size means snapshots from
    different terminals compare cleanly in CI / the garden TUI.
    """
