...
The installer's job is to add bonsai-cc's hook entries to the user's
existing Claude Code settings *without disturbing anything else*.
Every other configured hook, MCP server, and free-form setting must
survive byte-equivalently. Re-running the installer must be
idempotent. Uninstall must round-trip.

Marker convention
-----------------
Every entry the installer writes carries the JSON key
``"_bonsai_cc": true`` at the matcher-group level. Uninstall scans
for that marker. No other state is needed.

Settings shape
--------------
Claude Code's hook structure is::

    {
      "hooks": {
        "PostToolUse": [
          {
            "matcher": "Bash|Edit",         // optional; empty = all
            "hooks": [
...
We append one matcher group per registered event::

    {
      "_bonsai_cc": true,
      "matcher": "",                        // fire on all tools
      "hooks": [
        {"type": "command", "command": "<python> <hook_client_path>"}
      ]
    }
"""
...
# The events we register hooks for. Matches DESIGN.md §7.1. Adding a
