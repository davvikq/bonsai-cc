...
def _redact(node: object) -> None:
    """Walk ``node`` and replace any key in :data:`_REDACT_KEYS`.

    Recursive but bounded by the payload's own depth -- Claude Code's
    hook schema is shallow (event -> tool_input -> primitives), so
    this is microseconds. Mutates dict/list in place to avoid
    duplicating the payload, which keeps the 500 ms budget
    comfortable even on a 50 KB Edit payload.
    """
