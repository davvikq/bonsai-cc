...
# Bounded-state contract from the design contract The apply path treats
...
def apply_event(
    ...
    # turns "the session itself" into trunk growth.
...
def _extend_trunk(state: TreeState, event_idx: int) -> TreeState:
    ...
    a render-time concern; state only stores
    the base glyph.
    """
    if not state.trunk:
...
def _append_leaf(
    ...
    *,
    ...
    ``canopy_density`` instead The renderer
...
def _wither(
    ...
    entirely (the "leaf falls" behaviour from the design contract).
    """
    if not state.branches:
