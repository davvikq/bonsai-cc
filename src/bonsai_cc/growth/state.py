"""Pure tree state: the contract between growth and render.

This module is intentionally a flat collection of dataclasses with no
transformation logic. The growth engine produces these instances; the
renderer reads them. The renderer must never mutate them.

Coordinate system
-----------------
Logical ``(x, y)`` with **y increasing upward** (botanically
natural)::
...
The trunk base sits at logical origin ``(0, 0)``. Branches and
leaves have positive y; roots have negative y. The renderer
(:mod:`bonsai_cc.render.projection`) translates this into screen
``(col, row)`` where row increases downward.

Determinism contract
--------------------
Same ``session_seed`` + same event sequence → byte-identical
``TreeState``. See DESIGN.md §2.3. The pure data here participates
in that contract: no random state lives in ``TreeState`` itself; all
stochastic choices live in the growth engine.
"""
...
class Segment:
    ...
    look thicker, per DESIGN.md §2.2 step 6).
    """
...
class Branch:
    ...
    The ``file_path`` is the normalized identity key (see DESIGN.md
    §2.1 "file-path identity"). ``angle_deg`` is measured from
    ...
    canopy_density: int = 0  # see DESIGN.md §2.7
...
class Offshoot:
    ...
    Intentionally smaller than a regular branch (see DESIGN.md §2
    event mapping).
    """
...
def demo_tree(session_id: str = "demo") -> TreeState:
    ...
    # ``color=None`` lets the palette (DESIGN.md §2.6) pick the
