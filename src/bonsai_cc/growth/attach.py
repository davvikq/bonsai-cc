...
1. **Path identity** -- :func:`normalize_path` collapses any number of
   ...
   spawn two branches. The rules are pinned in DESIGN.md §2.1.
...
Code releases -- see DESIGN.md §3.2.
"""
...
def normalize_path(raw_path: str, cwd: str | None) -> str:
    ...
    Steps, per DESIGN.md §2.1:

    1. If ``raw_path`` is relative, resolve against ``cwd`` (when
       known). Falls through to ``Path.cwd()`` when ``cwd`` is None
       so tests stay deterministic.
    2. ``os.path.realpath`` to collapse symlinks. Tolerates the file
       not existing -- realpath returns its best guess.
    3. ``os.path.normpath`` to canonicalise separators.
    4. On Windows, lowercase the drive letter only.

    Failure falls back to the raw string verbatim -- DESIGN.md
    explicitly prefers a duplicated branch over a dropped event.
    ...
    try:
        ...
        if sys.platform == "win32" and len(normed) >= 2 and normed[1] == ":":
            normed = normed[0].lower() + normed[1:]
        return normed
...
def _tool_intent(event: PostToolUseEvent) -> AttachmentIntent:
    ...
    # Unknown tool: generic leaf on the most-recent branch (DESIGN.md §3.2).
    return AttachmentIntent(
        kind=AttachmentKind.ADD_LEAF,
        file_path_key=None,
        raw_tool_name=tool,
