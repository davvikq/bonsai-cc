...
Every random choice the growth engine makes flows through one of the
helpers here. They are **referentially transparent**: same
``(session_seed, event_idx)`` → same outputs, on every Python
version we support.

Why blake2b
-----------
``random.Random(seed)`` is technically deterministic but the
internal Mersenne Twister state initialization changed between
Python 3.10 and 3.11. ``blake2b`` (also stdlib) is cross-version
stable bit-for-bit. We hash ``(seed, event_idx)`` to an int and feed
that to ``Random``. The Mersenne Twister output from a fixed int
seed has been stable since Python 3.2 and the docs promise that;
this gives us the cross-machine determinism contract DESIGN.md §2.3
requires.

What's in this module
---------------------
* :func:`event_rng` -- the seeded RNG for one event.
* :func:`pick_one` / :func:`weighted_choice` -- deterministic
  selectors that don't rely on dict iteration order.
* :func:`angle_for` -- stable angle hashing for a string key
  (e.g. a file path, a cwd). Quantized to 0.01 rad as DESIGN.md
  §2.3 stipulates.
* :func:`branch_glyph_for_angle` -- pick a glyph based on direction.

No production rule expansion lives here yet. Recursive sub-branching
may come later; for now the growth pipeline uses the
incremental-attachment flavour described in DESIGN.md §2.2.
"""
...
# Quantization step (degrees) for any persisted angle. DESIGN.md §2.3
